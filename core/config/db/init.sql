...
BEGIN
	...
	Set Level=0 ;
	INSERT into menu_templst SELECT id,Level,Menu.order FROM Menu WHERE pid=rootid;
	WHILE ROW_COUNT()>0 DO
		SET Level=Level+1 ;
		INSERT into menu_templst SELECT A.id,Level,concat(B.scort,A.order) FROM Menu A,menu_templst B
